...
- #### English
Create a class called Month with an attribute called “name” (which will store the name of the month of the year). Add 11 Month objects (each with its different attributes) in an ArrayList, except the object with an attribute “August”. Then, do the insertion in the place that corresponds to this month and show that the ArrayList keeps the correct order.

Convert the previous ArrayList into a HashSet and make sure that it does not allow duplicates.

Cycle through the list with a for and with an iterator.
