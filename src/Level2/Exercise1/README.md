...
- #### English
Create a class called Restaurant with two attributes: name (String) and score. Implement the necessary methods so that you cannot enter the same name with the same name and the same score in a HashSet created in the main application although there can be a restaurant with the same name but different score.
